...
const { getFsFileName, getUrlFileName, getCircularReplacer } = require('./helpers')

const payloadKey = '__NUXT__'

const extractPayload = function(html, route, base, timestamp, subFolders){
  ...
  const fileName = getUrlFileName({ base, route, extension: 'js', timestamp, subFolders })
  const integrity = `sha256-${crypto.createHash('sha256').update(payload).digest('base64')}`;
  const script = `<script defer src="${fileName}" crossorigin="anonymous" integrity="${integrity}" data-cfasync="false"></script>`;
  return {
...
const writePayload = function(payload, route, root, timestamp, subFolders){
  let subpath = root
  ...
  fs.writeFile(getFsFileName({ root, route, extension: 'json', timestamp, subFolders }), availableData ? JSON.stringify(availableData, getCircularReplacer()) : '{}', 'utf8', err => {
    if (err) return console.error(err)
  })
}
